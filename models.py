...
def extract_mcq_from_pdf(pdf_bytes: BytesIO):
    ...
    # Create a temporary CSV file for results
    temp_csv = tempfile.NamedTemporaryFile(delete=False, suffix='.csv').name
    
    # Use the JEEExamParser with BytesIO
    parser = JEEExamParser(pdf_bytes, temp_csv)
    exam_data = parser.parse_exam_pdf()
...
class JEEExamParser:
    ...
    def __init__(self, pdf_bytes: BytesIO, output_csv: str):
        """Initialize with PDF bytes and output CSV path."""
    ...
    def parse_exam_pdf(self):
        """Parse the entire PDF and extract all question data."""
        logger.info(f"Parsing PDF: {self.pdf_bytes}")
